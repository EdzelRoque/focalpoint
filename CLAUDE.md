...
4. Backend `data/classification.js` checks Redis for a cached decision keyed on `sha256(url:sessionGoal:blockSensitivity)` (24h TTL). On miss, it calls Anthropic (`claude-haiku-4-5`) with sensitivity-specific system rules (`lenient` / `standard` / `strict`) and expects a strict JSON `{decision, reason}` reply. On any parse/API failure, it defaults to `ALLOW`.
...
- When a route has both mocked-auth tests (auth is incidental) and a real-auth test (token-binding is the contract), put the real-auth tests in a dedicated file named `routes/<routeFile>.auth.test.js` — e.g. `routes/sessionRoutes.auth.test.js`. Mixing both auth setups in one file produces a contradictory mock environment.
...
- When writing tests, match the actual current behavior, not assumed behavior. This means: don't write assertions against behavior the code doesn't have. It does NOT mean: avoid forward-looking regression guards on CAT-tier security boundaries. IDOR injection tests (e.g. sending `?userId=B` while holding a token for A) are valid even when the route currently ignores the injected field — the competing value is what makes the token-binding assertion meaningful, and the test guards against a future refactor that wires the injected field in.
